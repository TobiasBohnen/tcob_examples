...
void TileMapEx::on_fixed_update(milliseconds deltaTime)
{
    ...
    auto const  mouse {window().camera().convert_screen_to_world(locate_service<input::system>().mouse().get_position())};
...
void TileMapEx::on_key_down(keyboard::event const& ev)
{
    ...
    case scan_code::D5: {
        std::vector<sprite_animation::frame> frames = {{"stone1", 250ms}, {"dirt1", 250ms}, {"grass1", 250ms}, {"stone3", 250ms}, {"grass4", 250ms}};
        sprite_animation                     ani {frames};
        _tween = std::make_shared<sprite_animation_tween>(ani.duration(), ani);
