...
void B2DDebugDraw::draw_circle(point_f center, f32 radius, color color)
{
    ...
    _canvas.set_stroke_style(color);
    _canvas.begin_path();
    _canvas.circle(center * physicsWorldSize, radius * physicsWorldSize);
    _canvas.stroke();
}
...
{
    _canvas.set_stroke_width(radius * physicsWorldSize * 2);
    _canvas.set_stroke_style(color);
    ...
    _canvas.begin_path();
    _canvas.circle(p1 * physicsWorldSize, radius * physicsWorldSize);
    _canvas.circle(p2 * physicsWorldSize, radius * physicsWorldSize);
    _canvas.fill();
}
...
void B2DDebugDraw::draw(physics::world const& world, f32 alpha, gfx::render_target& target)
{
    ...
    _debugRenderer.render_to_target(target, transform::Identity);
}
