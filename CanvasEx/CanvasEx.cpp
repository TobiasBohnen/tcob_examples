...
void CanvasEx::on_fixed_update(milliseconds /* deltaTime */)
{
    ...
    window().Title = std::format("TestGame | FPS avg:{:.2f} best:{:.2f} worst:{:.2f} | x:{} y:{} | press 'm' to cycle examples",
...
void CanvasEx::on_draw_to(render_target& target, transform const& xform)
{
...
void CanvasEx::canvas_ray_cast()
{
    ...
    auto castRay {
        [&](auto&& ray) {
            ray.intersect_circle(_mp, size, [&](ray::result const& r) { points.push_back(r); });
            ...
            ray.intersect_rect(topLeft, topRight, bottomLeft, bottomRight, [&](ray::result const& r) { points.push_back(r); });
            _canvas.begin_path();
