...
void UniformBufferEx::on_start()
{
    ...
    sprite0.Bounds   = {{400, 300}, {256, 256}};
}
...
{
    _elapsed += deltaTime;
    f32 const t {static_cast<f32>(_elapsed.count() / 1000.0f)};

    std::array<f32, 4> const brightness {
        0.5f + (0.5f * std::sin(t)),
        static_cast<f32>(0.5f + (0.5f * std::sin(t + (TAU / 3.0f)))),
        static_cast<f32>(0.5f + (0.5f * std::sin(t + (2.0f * TAU / 3.0f)))),
        1.0f};
    _ubo0->update(std::span<f32 const> {brightness}, 0);
